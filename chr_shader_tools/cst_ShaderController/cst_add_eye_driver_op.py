...
class CST_OT_addEyeDrivers_OP(Operator):
    ...
    def add_drivers(self, eye, is_left_eye: bool, wm):
        ...
        # Location X
        eye.driver_remove('["position_scale' + suffix + '"]', 0)
        ...
        # Location Y
        eye.driver_remove('["position_scale' + suffix + '"]', 1)
        ...
        # Scale X
        eye.driver_remove('["position_scale' + suffix + '"]', 2)
        ...
        # Scale Y
        eye.driver_remove('["position_scale' + suffix + '"]', 3)
        ...
        # Rotation Z
        eye.driver_remove('["rotation' + suffix + '"]')
...
class CST_OT_addEyeDriversNoUpDown_OP(Operator):
    bl_idname = "object.add_eye_drivers_no_up_down"
    bl_label = "Link eyes to a rig w/o up/down"
    bl_description = "Adds drivers from Rig which does not have eye_up/eye_down bones to both eyes, so that the shader can access them."
    bl_options = {"REGISTER", "UNDO"}
    ...
    def add_drivers(self, eye, is_left_eye: bool, wm):
        ...
        driver.expression = "(0.5 + (-50)*target)"
...
def register():
    ...
    bpy.utils.register_class(CST_OT_addEyeDriversNoUpDown_OP)

def unregister():
    bpy.utils.unregister_class(CST_OT_addEyeDriversNoUpDown_OP)
