...
class FDG_OT_GenerateDrivers_Op(Operator):
    ...
    def execute(self, context):
        ...
        head = bpy.context.scene.object1
        head_bone_name = bpy.context.scene.bone1
        cam = bpy.context.scene.object2
        cam_bone_name = bpy.context.scene.bone2
        prop_holder = bpy.context.scene.propertyObject

        if prop_holder is not None and head is not None and cam is not None:
            ...
            if head.type == 'ARMATURE' and head_bone_name != '':
                head_empty.location = head.location + \
                    head.data.bones[head_bone_name].head
            else:
                head_empty.location = head.location

            bpy.context.collection.objects.link(head_empty)

            parent_objects(head, head_empty, head_bone_name)
            ...
            edit_bones = head.data.edit_bones
            ...
            hidden_controller_pose_bone = head.pose.bones[hidden_controller_bone_name]
            visible_controller_pose_bone = head.pose.bones[visible_controller_bone_name]
            ...
            add_var(driver_normalize, head, "angle_x", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driven_Props"]["angle_x"]')
            add_var(driver_normalize, head, "angle_z", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driven_Props"]["angle_z"]')
            add_var(driver_normalize, head, "bias", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["bias"]')

            # add variables to the shapes drivers
            # LEFT
            add_var(driver_shapes_left, head, "angle_z", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driven_Props"]["angle_z"]')
            add_var(driver_shapes_left, head, "bias", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["bias"]')
            add_var(driver_shapes_left, head, "blend", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["blend"]')
            add_var(driver_shapes_left, head, "blendUpDown", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["blendUpDown"]')
            add_var(driver_shapes_left, head, "upDownNormalize", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driven_Props"]["upDownNormalize"]')

            # RIGHT
            add_var(driver_shapes_right, head, "angle_z", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driven_Props"]["angle_z"]')
            add_var(driver_shapes_right, head, "bias", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["bias"]')
            add_var(driver_shapes_right, head, "blend", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["blend"]')
            add_var(driver_shapes_right, head, "blendUpDown", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driver_Controller"]["blendUpDown"]')
            add_var(driver_shapes_right, head, "upDownNormalize", type='SINGLE_PROP',
                    rna_data_path='pose.bones["Driven_Props"]["upDownNormalize"]')
            ...
            self.report({'WARNING'}, "Please select all Objects!")
            return {'CANCELLED'}
